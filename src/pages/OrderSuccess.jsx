  ...
  return (
    <div className="flex flex-col items-center justify-center py-8 animate-fadeIn w-full max-w-5xl mx-auto px-4 print:py-0">
       <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 sm:p-10 mb-12 w-full max-w-2xl flex flex-col items-center">
          ...
          <div className="w-full bg-slate-50 border border-slate-200 rounded-xl p-5 sm:p-7 mb-8 text-left">
               <div className="flex justify-between items-start mb-6 border-b pb-4"><div><h3 className="font-bold text-slate-800 text-lg flex items-center gap-2"><FiPackage className="text-indigo-600"/> Order #{placedOrder.id}</h3><p className="text-xs text-slate-500 mt-1">{new Date(placedOrder.date).toLocaleString()}</p></div><button onClick={() => window.print()} className="text-sky-600 bg-sky-50 px-3 py-1.5 rounded flex items-center gap-2 text-sm font-bold print:hidden"><FiPrinter/> Print</button></div>
               <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar print:max-h-none">{placedOrder.items.map((item, i) => (<div key={i} className="flex gap-4 items-center bg-white p-3 rounded border border-slate-100"><img src={item.product.images[0]} className="w-14 h-14 rounded object-cover border" /><div className="flex-1 min-w-0"><p className="font-bold text-slate-800 text-sm truncate">{item.product.name}</p><p className="text-xs text-slate-500">Qty: {item.quantity}</p></div><p className="font-bold text-slate-900 text-sm">₹{(item.product.price * item.quantity).toFixed(2)}</p></div>))}</div>
               <div className="mt-6 pt-4 border-t-2 border-dashed border-slate-200">{placedOrder.shippingAddress && (<div className="mb-4 bg-white p-3 rounded border"><p className="text-xs font-bold text-slate-400 uppercase mb-1">Shipping To</p><p className="text-sm text-slate-800">{placedOrder.shippingAddress.address}, {placedOrder.shippingAddress.city}</p></div>)}<div className="flex justify-between text-sm mb-2"><span>Subtotal</span><span>₹{(placedOrder.total / 1.08).toFixed(2)}</span></div><div className="flex justify-between text-sm mb-3"><span>Tax (8%)</span><span>₹{(placedOrder.total - (placedOrder.total / 1.08)).toFixed(2)}</span></div><div className="flex justify-between font-extrabold text-lg text-slate-900 pt-3 border-t"><span>Total Paid</span><span className="text-indigo-700">₹{placedOrder.total.toFixed(2)}</span></div></div>
          </div>
          <div className="flex gap-3 w-full justify-center print:hidden"><button onClick={() => navigate('/profile')} className="border border-slate-300 text-slate-700 bg-white px-5 py-2.5 rounded-lg text-sm">Track Order</button><button onClick={() => navigate('/')} className="flex justify-center items-center gap-2 bg-slate-900 text-white px-5 py-2.5 rounded-lg text-sm"><FiHome /> Back to Store</button></div>
       </div>
       <div className="w-full print:hidden"><div className="flex justify-between items-end mb-6 border-b pb-2"><h3 className="text-2xl font-bold text-slate-900">Recommended for You</h3><button onClick={() => navigate('/')} className="text-sky-600 text-sm">See all</button></div><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">{recommendedProducts.map(p => (<ProductCard key={p.id} product={p} />))}</div></div>
    </div>
  );
};
