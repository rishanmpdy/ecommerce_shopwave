         ...
         <div style={{ maxWidth: 1200, margin: '0 auto', padding: '20px 28px', display: 'grid', gridTemplateColumns: '1fr 360px', gap: 18, alignItems: 'start' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
               ...
               <div style={{ background: '#fff', borderRadius: 8, padding: 24, boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
                  ...
                  <div className="mb-6 space-y-3"><div className="grid grid-cols-1 md:grid-cols-3 gap-3">{addresses.map((addr) => (<label key={addr.id} className={`border rounded-xl p-4 cursor-pointer transition-all flex flex-col ${selectedAddressIndex === addr.id ? 'border-indigo-600 bg-indigo-50 ring-1 ring-indigo-600' : 'border-slate-200'}`}><div className="flex items-center justify-between mb-2"><span className="font-bold text-slate-800">{addr.title}</span><input type="radio" checked={selectedAddressIndex === addr.id} onChange={() => setSelectedAddressIndex(addr.id)} className="text-indigo-600" /></div>{addr.address && <span className="text-xs text-slate-500 truncate">{addr.address}</span>}</label>))}</div></div>
                  <form id="checkout-form" onSubmit={handlePayment} className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                     ...
                     {selectedAddressIndex === addresses[addresses.length - 1]?.id && (<div className="md:col-span-2 mb-2"><div className="flex gap-3">{['Home', 'Office', 'Other'].map((type) => (<label key={type} className={`flex-1 text-center py-2.5 rounded-xl border text-sm font-bold cursor-pointer transition-all ${formData.title === type ? 'border-indigo-600 bg-indigo-50 text-indigo-700' : 'border-slate-200'}`}><input type="radio" name="title" value={type} checked={formData.title === type} onChange={handleInputChange} className="sr-only" required />{type}</label>))}</div></div>)}
                     <input type="text" name="firstName" value={formData.firstName || ''} onChange={handleInputChange} placeholder="First Name" className="p-3 border rounded-lg bg-slate-50" /><input type="text" name="lastName" value={formData.lastName || ''} onChange={handleInputChange} placeholder="Last Name" className="p-3 border rounded-lg bg-slate-50" /><input type="text" name="address" value={formData.address || ''} onChange={handleInputChange} placeholder="Address" className="md:col-span-2 p-3 border rounded-lg bg-slate-50" /><input type="text" name="city" value={formData.city || ''} onChange={handleInputChange} placeholder="City" className="p-3 border rounded-lg bg-slate-50" /><div className="grid grid-cols-2 gap-2"><input type="text" name="state" value={formData.state || ''} onChange={handleInputChange} placeholder="State" className="p-3 border rounded-lg bg-slate-50" /><input type="text" name="zip" value={formData.zip || ''} onChange={handleInputChange} placeholder="PIN" className="p-3 border rounded-lg bg-slate-50" /></div>
                  </form>
               </div>
               ...
               <div style={{ background: '#fff', borderRadius: 8, padding: 24, boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
                  ...
                  <div className="space-y-4">
                     {['upi', 'card', 'netbanking', 'cod'].map(method => (
                        <label key={method} className={`border rounded-xl p-4 cursor-pointer transition-all flex flex-col ${paymentMethod === method ? 'border-primary-500 bg-primary-50 ring-1 ring-primary-500' : 'border-slate-200'}`}>
                           <div className="flex items-center gap-3"><input type="radio" value={method} checked={paymentMethod === method} onChange={(e) => setPaymentMethod(e.target.value)} className="text-primary-600" />{method === 'upi' && <FiSmartphone className="text-primary-600" />}{method === 'card' && <FiCreditCard className="text-primary-600" />}{method === 'netbanking' && <FiMonitor className="text-primary-600" />}<span className="font-bold text-slate-800 uppercase">{method}</span></div>
                        </label>
                     ))}
                  </div>
               </div>
            ...
            <div style={{ background: '#fff', borderRadius: 8, padding: 24, boxShadow: '0 1px 4px rgba(0,0,0,0.07)', position: 'sticky', top: 24 }}>
               ...
               <div className="space-y-4 mb-6 max-h-[300px] overflow-y-auto border-b pb-6">{displayItems.map((item, i) => (<div key={i} className="flex gap-4 items-center"><img src={item.product.images[0]} style={{ width: 60, height: 75, objectFit: 'cover' }} /><div className="flex-1 min-w-0"><p className="font-semibold text-sm truncate">{item.product.name}</p><p className="text-xs text-slate-500">Qty: {item.quantity}</p></div><p className="font-bold text-sm">₹{(item.product.price * item.quantity).toFixed(2)}</p></div>))}</div>
               ...
               <div className="border-y py-4 mb-6 font-bold flex justify-between"><span>Total</span><span>₹{(displayTotal + (displayTotal * 0.08)).toFixed(2)}</span></div>
