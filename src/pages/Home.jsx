...
import { useProducts, useDeals } from '../features/products/useProducts';
    ...
    <>
      ...
      <div className="sh">
        ...
        <div className="sh-strip">
          Get <span>FREE delivery</span> on orders above ₹1000 &nbsp;|&nbsp; <span>New Collection 2026</span> is Live!
        </div>
        ...
        <section style={{ position: 'relative', width: '100%', height: 'clamp(340px,52vw,580px)', overflow: 'hidden', background: '#0F172A' }}>
          ...
          <div style={{ position: 'absolute', inset: 0, zIndex: 5, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 clamp(20px,7vw,96px)', maxWidth: 620 }}>
            <span style={{ background: BANNER.accent, color: '#fff', fontFamily: 'Poppins,sans-serif', fontWeight: 700, fontSize: 11, letterSpacing: '0.14em', textTransform: 'uppercase', padding: '4px 14px', borderRadius: 2, marginBottom: 14, display: 'inline-block', width: 'fit-content' }}>
              {BANNER.tag}
            </span>
            <h1 style={{ fontFamily: 'Poppins,sans-serif', fontWeight: 800, fontSize: 'clamp(1.8rem,5vw,3.6rem)', color: '#fff', margin: '0 0 12px', lineHeight: 1.12, whiteSpace: 'pre-line' }}>
              {BANNER.headline}
            </h1>
            <p style={{ fontFamily: 'Inter,sans-serif', fontSize: 'clamp(.85rem,1.3vw,1rem)', color: 'rgba(255,255,255,0.82)', margin: '0 0 26px', maxWidth: 400, lineHeight: 1.65 }}>
              {BANNER.sub}
            </p>
            <button className="sh-btn-buy" onClick={() => navigate('/shop')} style={{ width: 'auto', alignSelf: 'flex-start', padding: '12px 30px', borderRadius: 4, fontSize: 14 }}>
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 clamp(10px,2.5vw,28px)' }}>
              ...
              <div key={b.title} className="sh-card" style={{ cursor: 'pointer' }} onClick={() => navigate('/shop')}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', minHeight: 170 }}>
                  <div style={{ background: b.bg, padding: '22px 18px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    ...
                    <div style={{ alignSelf: 'flex-start', background: '#111', color: '#fff', borderRadius: 3, padding: '8px 16px', fontSize: 12, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 5 }}>
          ...
          <div className="sh-card" style={{ marginBottom: 14 }}>
            <div className="sh-card-pad">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
                <h2 className="sh-sec-title" style={{ margin: 0 }}>
                  Today's Deals &nbsp;
                  <span style={{ background: '#E11D48', color: '#fff', fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 2 }}>Limited Time</span>
                </h2>
              ...
              <div className="sh-hscroll">
                {isDealsLoading ? (
                  <div style={{ display: 'flex', gap: 14 }}>
                    {[1, 2, 3, 4].map((n) => (
                      <div key={n} style={{ minWidth: 200, height: 260, background: '#f5f5f5', borderRadius: 8, animation: 'pulse 1.5s infinite ease-in-out' }} />
                    ))}
                  </div>
                ) : (
                  deals?.map((d) => (
                    <div key={d.name} className="sh-deal" onClick={() => navigate('/shop')}>
                      <div style={{ position: 'relative' }}>
                        ...
                        <span className={`sh-badge ${d.cls}`} style={{ position: 'absolute', top: 8, left: 8, margin: 0 }}>{d.badge}</span>
                      </div>
                      <div className="sh-deal-body">
                        <span className="sh-off">{d.off}</span>
                        <div><span className="sh-price">{d.price}</span><span className="sh-original">{d.orig}</span></div>
                        <p style={{ fontSize: 12, color: '#333', margin: '5px 0 0', fontWeight: 500, lineHeight: 1.3 }}>{d.name}</p>
                      </div>
                    </div>
                  ))
                )}
          ...
          <div className="sh-card" style={{ marginBottom: 14 }}>
            <div className="sh-card-pad">
              ...
              {isProductsLoading ? (
                ...
                <div className="sh-prod-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
                  {trendingProducts.map((product) => (
                    <div key={product.id} className="sh-prod-wrap">
              ...
              <div style={{ display: 'flex', justifyContent: 'center', marginTop: 20 }}>
                <button className="sh-btn-add" onClick={() => navigate('/shop')} style={{ padding: '11px 40px', borderRadius: 4, fontSize: 14 }}>
          ...
          <div className="sh-card" style={{ marginBottom: 14, cursor: 'pointer' }} onClick={() => navigate('/shop')}>
            <div style={{ position: 'relative', height: 190 }}>
              <img
                src="/src/assets/products/stephome5.jpg"
                alt="Offer Banner"
              ...
              <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 36px' }}>
                ...
                <button className="sh-btn-buy" style={{ width: 'auto', alignSelf: 'flex-start', borderRadius: 4, padding: '9px 26px', fontSize: 13 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: 0 }}>
              {TRUST_ITEMS.map((t, i, arr) => (
                <div key={t.title} style={{ padding: '16px 18px', borderRight: i < arr.length - 1 ? '0.5px solid #E8E8E8' : 'none', display: 'flex', gap: 11, alignItems: 'center' }}>
                  ...
                  <div>
                    <div style={{ fontFamily: 'Poppins,sans-serif', fontWeight: 600, fontSize: 13, color: '#111' }}>{t.title}</div>
